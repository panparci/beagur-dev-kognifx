...
func scanBankLines(rows pgx.Rows) ([]BankTransactionLine, error) {
	...
	for rows.Next() {
		...
		var matchedDon, matchedLed, sugDon, sugLed, reviewed *string
		...
		if err := rows.Scan(
			...
			&matchedDon, &matchedLed, &sugDon, &sugLed, &reviewed, &reviewedAt,
		); err != nil {
			return nil, err
		}
...
func (s *Store) CreateBankUpload(ctx context.Context, in CreateBankUploadInput) (BankStatementUpload, error) {
	...
	for _, raw := range in.Lines {
		...
		status := "UNMATCHED"
		var matchedDon, matchedLed, sugDon, sugLed *string
		if dir == "INCOMING" {
			status, matchedDon, sugDon = matchIncoming(raw, pending)
		} else {
		...
		_, err = tx.Exec(ctx, `
			...
			ptrStr(matchedDon), ptrStr(matchedLed), ptrStr(sugDon), ptrStr(sugLed),
		)
		if err != nil {
			return BankStatementUpload{}, err
		}
...
func matchIncoming(line BankLineInput, pending []pendingDonationMatch) (status string, matched, suggested *string) {
