...
## Bea Guru project (mandatory override)

This monorepo uses **Tailwind CSS v4** with `@tailwindcss/vite` — not the CDN.

| Item | Path |
|------|------|
| Entry CSS | `fe/src/styles/app.css` |
| Theme tokens (`bea-*`) | `@theme` block in `app.css` |
| Portal component classes | `fe/src/styles/portal-ui.css` |
| Landing animations/panels | `fe/src/styles/landing.css` |
| Vite plugin | `fe/vite.config.ts` |

When adding utilities, prefer Tailwind classes + existing `bea-*` theme colors. Keep portal/landing custom CSS in the stylesheets above; do not reintroduce the Tailwind CDN. Pair with `ui-ux-pro-max` skill for Bea Guru visual rules.
